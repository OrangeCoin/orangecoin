...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to orangecoin server / RPC client
            std::string strUsage = _("orangecoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  orangecoin [options]                     " + "\n" +
                  "  orangecoin [options] <command> [params]  " + _("Send command to -server or orangecoin") + "\n" +
                  "  orangecoin [options] help                " + _("List commands") + "\n" +
                  "  orangecoin [options] help <command>      " + _("Get help for a command") + "\n";
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -pid=<file>            " + _("Specify pid file (default: orangecoin.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 14377 or testnet: 14378)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 14376)") + "\n" +
...
//    }
    ...
    // Make sure only a single orangecoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  orangecoin is probably already running."), GetDataDir().string().c_str()));
    ...
    // requested to kill orangecoin-qt during the last operation. If so, exit.
